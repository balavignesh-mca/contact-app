...
export const Welcome = () => {
  ...
  async function fetchContacts() {
    try {
      let response = await axios.get(
        "http://localhost:4000/api/getContacts",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
          },
    ...
    if (!contactName) {
      ...
      try {
        const response = await axios.post(
          "http://localhost:4000/api/createContact/",
          {
            name: contactName,
            phone: mobileNo,
            phone1: mobileNo1,
    ...
    if (!contactName) {
      ...
      try {
        const response = await axios.put(
          `http://localhost:4000/api/updateContact/${id}`,
          {
            name: contactName,
            phone: mobileNo,
            phone1: mobileNo1,
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:4000/api/deleteContact/${id}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
